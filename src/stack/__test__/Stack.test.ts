import {Stack} from '../StackClass';

interface key_value_string {value:String,key:String};

describe('3. Stack Class Implementation', () => {
    /**
     * TEST_1
     */
    test('TEST_1:Create an empty stack', () => {
      const stack = new Stack();
  
      expect(stack).not.toBeNull();
    });
    /**
     * TEST_2
     */
    test('TEST_2:Push data to stack', () => {
      const stack = new Stack<key_value_string>();
  
      stack.push({ value: 'test1', key: 'key1' });
      stack.push({ value: 'test2', key: 'key2' });
  
      expect(stack.top.data['value']).toBe('test2');
      expect(stack.top.next.data['value']).toBe('test1');
      expect(stack.bottom.data['value']).toBe('test1');
      expect(stack.bottom.next).toBeNull();
    });

    /**
     * TEST_3
     */
    test('TEST_3:Remove data from stack', () => {
      const stack = new Stack<number>();
  
      stack.push(1);
      stack.push(2);
      stack.push(3);
  
      expect(stack.pop().data).toBe(3);
      expect(stack.pop().data).toBe(2);
      expect(stack.pop().data).toBe(1);
      expect(stack.pop()).toBeNull();
    });
  
    test('TEST_4:peek node from stack', () => {
      const stack = new Stack<number>();
  
      stack.push(1);
      stack.push(2);
      stack.push(3);
  
      expect(stack.peek().data).toBe(3);
    });
  });