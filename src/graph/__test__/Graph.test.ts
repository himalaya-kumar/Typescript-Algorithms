...
describe('11. Graphs Implementation', () => {
  test('TEST_1:Create an empty graph', () => {
    const graph = new Graph();

    expect(graph).toBeDefined();
  });

  test('TEST_2:Add a new vertex to the graph', () => {
    const graph = new Graph();
  ...
  test('TEST_3:Throws error if vertex already exists', () => {
    try {
      const graph = new Graph();
      ...
      expect(error).toHaveProperty('message', 'the node already exist');
    }
  });

  test('TEST_4:Add edges to graph', () => {
    const graph = new Graph();
